    ...
    public class ToDoesController : Controller
    {
        ...
        private IEnumerable<ToDo> GetMyTodoes()
        {
            ...
            else
            {
                ...
                myToDoes = db.ToDos.ToList().Where(x => x.User == currentUser);
                int completeCount = 0;
        ...
        public ActionResult AJAXCreate([Bind(Include = "Id,Description")] ToDo toDo)
        {
            if (ModelState.IsValid)
            {
                ...
                toDo.User = currentUser;
                toDo.IsDone = false;
                toDo.Category = db.Categories.First();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,Description,isDone,CategoryId")] ToDo toDo)
        {

            if (ModelState.IsValid)
            {
                db.Entry(toDo).State = EntityState.Modified;                
                Category cat = db.Categories.Where(x => x.CategoryId == toDo.CategoryId).First();
                toDo.Category.CategoryId = cat.CategoryId;                               
                db.SaveChanges();
                return RedirectToAction("Index");
            }
